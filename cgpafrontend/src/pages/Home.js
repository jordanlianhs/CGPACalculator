...
export default function Home() {
      ...
      for (const [key, coursesGroup] of sortedGroupedCourses) {
        ...
        try {
          const sgpaData = await axios.get(
            `https://34.143.202.68:8080/calculateSGPA/${year}/${sem}`
          );
          coursesGroup.sgpa = sgpaData.data;
          const sCredit = await axios.get(
            `https://34.143.202.68:8080/calculateSCredit/${year}/${sem}`
          );
  ...
  const loadCourses = async () => {
    const result = await axios.get("https://34.143.202.68:8080/getCourses");
    setCourses(result.data);
    console.log(result.data);
  };
  const loadCgpa = async () => {
    const result = await axios.get("https://34.143.202.68:8080/calculateCGPA");
    setCgpa(result.data);
  };
  const loadTotalCredits = async () => {
    const result = await axios.get("https://34.143.202.68:8080/getTotalCredits");
